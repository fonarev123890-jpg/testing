...
test("5. Проверка элемента с несколькими классами", async ({ page }) => {
  ...
  const multi = page.locator("#multi-class");
  await expect(multi).toHaveClass(/warning/);
  await expect(multi).toHaveClass(/box/);
  await expect(multi).toHaveClass(/large/);
  await expect(multi).toHaveClass(/rounded/);

  await page.getByRole("button", { name: "Изменить классы" }).click();
  await expect(multi).not.toHaveClass(/warning/);
  await expect(multi).toHaveClass(/error/);
  await expect(multi).not.toHaveClass(/large/);
  await expect(multi).toHaveClass(/rounded/);
  await expect(multi).toHaveClass(/box/);
});
