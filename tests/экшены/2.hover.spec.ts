...
test.describe("Базовые hover-эффекты", () => {
  ...
  test("Hover события логируются", async ({ page }) => {
    ...
    hoverBox.hover();
    await expect(page.locator("#hover-log")).toContainText(
      "Наведение на простой блок",
    );

    await page.mouse.move(0, 0);
    await expect(page.locator("#hover-log")).toContainText(
      "Уход с простого блока",
    );
  });
});
...
test.describe("Всплывающие подсказки", () => {
  ...
  test("Подсказка правильно позиционируется", async ({ page }) => {
    ...
    if (tooltipBox && box) {
      ...
      expect(
        Math.abs(tooltipBox.x + tooltipBox.width / 2 - (box.x + box.width / 2)),
      ).toBeLessThan(2);
    }
  });
});
